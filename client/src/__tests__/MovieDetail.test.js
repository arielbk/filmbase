...
import { CAST_PATH } from '../components/Cast';
...
test('<MovieDetail /> receives failed api response', async () => {
	...
	fetch.mockResponseOnce(JSON.stringify(movie)).mockResponseOnce(JSON.stringify(credits));
...
test('<MovieDetail /> receives valid api response', async () => {
	...
	expect(getByTestId('movie-poster').getAttribute('src')).toBe(
	...
	expect(genreTabs[genreTabs.length - 1].innerHTML).toBe(
	...
	expect(queryAllByTestId('credit-photo')[0].getAttribute('src')).toBe(
		`${CAST_PATH}${credits.cast[0].profile_path}`
	);
